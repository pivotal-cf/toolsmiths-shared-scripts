...
BOSH_GUID=$(curl -k "https://pcf.${ENV_NAME}.cf-app.com/api/v0/staged/products" -X GET -H "Authorization: Bearer $UAA_ACCESS_TOKEN" | jq '.[] | select(.installation_name=="p-bosh")' | jq -r .guid)
